...
export function planFromGumroadProduct(
  ...
  if (unlimitedId && pid === unlimitedId) {
    return "unlimited"
  }
  if (proId && pid === proId) {
    return "pro"
  }
  if (link.includes("industry-ai-news-unlimited")) {
    return "unlimited"
  }
  if (link.includes("industry-ai-news-pro")) {
    return "pro"
  }
  return null
}
...
export function isDeactivatePing(
  refunded: string | boolean | undefined,
  cancelled: string | boolean | undefined,
  ended: string | boolean | undefined
): boolean {
