    ...
    body: (
      <Tips>
        <div className="space-y-3">
          ...
          <p>未配置 seller_id 时，Ping 返回「Gumroad integration is not configured」。</p>
        </div>
      </Tips>
    ),
  },
    ...
    body: (
      <Tips>
        <div className="space-y-3">
          ...
          <p>禁用后，公开上色、超分、物体移除、专业证件照接口将对所有应用返回服务不可用。</p>
        </div>
      </Tips>
    ),
  },
...
export function IntegrationsSettingsList({ appId }: { appId: string }) {
  ...
  return (
    <>
      <SectionCard
        title="集成渠道"
        tips="全站共用；列表可快速禁用或打开编辑。未完成时请补全凭证并勾选启用。"
      >
        {!overview ? (
